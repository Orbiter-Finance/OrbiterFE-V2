...
export default {
    ...
    async getLpTokenInfo(fromChainID, tokenAddress, count = 10) {
        const theLpTokenInfo = this.getLpTokenInfoOnce(
            fromChainID,
            tokenAddress
        )
        if (theLpTokenInfo) {
    ...
    getLoopringBalance: async function (
        ...
        try {
            ...
            if (resp.status === 200) {
                ...
                const totalBalance = balanceMap.total
                    ? Number(balanceMap.total)
                    : 0
    ...
    sendTransfer: async function (
        ...
        const lpTokenInfo = await this.getLpTokenInfo(
            localChainID,
            tokenAddress
        )
