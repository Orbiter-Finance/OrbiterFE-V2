  ...
  try {
    ...
    for (let zk2Info of zk2AllTxList) {
      ...
      for (let j = 0; j < makerList.length; j++) {
        ...
        const theValue = new BigNumber(txinfo.value).multipliedBy(10 ** makerInfo.precision).toString()
        if (txinfo.from === _makerAddress) {
          ...
          const toChainId = orbiterCore.getToChainIDFromAmount(chainID, theValue)
          if (toChainId) {
            let arr1 = [makerInfo.c1ID, makerInfo.c2ID]
    ...
    if (req.state === 1) {
      ...
      if (supportChains.indexOf(4) > -1 || supportChains.indexOf(44) > -1) {
        allPromises.push(async () => {
          ...
          const { SNFromTxList, SNToTxList } = await getTransactionListStarknet(
            req.address,
            chainID,
            needTimeStamp,
            makerList
