...
export default {
    ...
    */
        ...
        const needTimeStamp =
            nowTimeStamp - 86400 * (req.daysAgo ? req.daysAgo : 7)
            ...
            try {
                ...
                for (const i in res.result) {
                    if (Object.hasOwnProperty.call(res.result, i)) {
                        ...
                        const txinfo =
                            TxInfo.getTxInfoWithEtherScan(etherscanInfo)
                        if (txinfo.from === req.maker) {
                            ...
                            for (const j in avalibleTimes) {
                                if (
                                    Object.hasOwnProperty.call(avalibleTimes, j)
                                ) {
        ...
        if (req.pool.c1ID === 3 || req.pool.c2ID === 3) {
            ...
            let zkTokenInfo
            try {
                zkTokenInfo = await thirdapi.getZKTokenInfo(
                    zkTokenInfoReq,
                    next
                )
            } catch (error) {
                            ...
                            if (Object.hasOwnProperty.call(zkList, i)) {
                                ...
                                if (timestamp >= needTimeStamp) {
                                    ...
                                    if (zk_txinfo.from === req.zkParam.maker) {
                                        ...
                                        zk_txinfo.to === req.zkParam.maker
                                    ) {
            ...
            try {
                ...
                for (const i in ArRes.result) {
                    if (Object.hasOwnProperty.call(res.result, i)) {
                        ...
                        const ArTxinfo =
                            TxInfo.getTxInfoWithEtherScan(ArScanInfo)
                    ...
                    if (Object.hasOwnProperty.call(fromList, j)) {
                        ...
                        if (isMatch) {
                            if (fromTxInfo.to === toTxInfo.from) {
                                ...
                                if (
                                    realToAmount * (1 - pool.fee / 10000) ===
                                    realFromAmount
                                ) {
                                    const toIndex = process.getIndexInArr(
                                        newToList,
                                        toTxInfo
                                    )
