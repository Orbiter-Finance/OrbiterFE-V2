<template>
<div ref="box" class="raiseup-select-box" :style="{display: visible ? 'block' : 'none'}">
  <div class="raiseup-select-box-wrapper">
    <div ref="items" class="items">
      <div v-for="item in showedDatas" :key="item.value" @click="itemClick(item)" :class="['item', {selected: isSelected(item)}]">
        <template v-if="item.icon">
          <img
            ...
            alt=""
          />
          <svg-icon v-else class="item-icon" :iconName="item.icon"></svg-icon>
        </template>
...
export default {
  ...
  computed: {
    showedDatas() {
      return this.datas.map(v => ({...v, value: v[this.keyMaps.value], label: v[this.keyMaps.label], iconType: v.iconType || this.iconType}))
    },
  },
  methods: {
    isSelected(item) {
