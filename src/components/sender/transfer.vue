  ...
  <o-box-content class="senderbody" style="width: 34.5rem">
    ...
    <div class="content">
      ...
      <div class="subContent">
        ...
        <div class="middleImge" @click="transfer_mid">
          <img
...
export default {
  ...
  asyncComputed: {
    async userMaxPrice() {
      ...
      let avalibleDigit = selectMakerInfo.precision === 18 ? 5 : 2;
      let userMax =
  ...
  computed: {
    SendEnAble() {
      ...
      if (new BigNumber(this.transferValue).comparedTo(new BigNumber(0)) < 0) {
        return "disabled";
      }
      if (
    ...
    SendBtnText() {
      ...
      if (this.isLogin) {
        ...
        if (transferValue.comparedTo(makerMax) > 0) {
          return "INSUFFICIENT LIQUIDITY";
        }
        if (transferValue.comparedTo(makerMin) < 0) {
          return "INSUFFICIENT FUNDS";
        }
        return "SEND";
      }
    ...
    toValueToolTip() {
      return "Sender will pay a 0.3% trading fee for each transfer.";
    },
    securityToolTip() {
    ...
    gasFeeToolTip() {
      return `<b>The cost before using Orbiter</b><br />Gas Fee: $${(
        this.$store.state.transferData.ethPrice * this.originGasCost
      )
        .toFixed(2)
        .toString()}<br /><br /><b>The cost after using Orbiter</b><br />Trading Fee: $${
        this.orbiterTradingFee
      }<br />Withholding Gas Fee: $${
        this.$store.getters.realSelectMakerInfo
          ? this.$store.getters.realSelectMakerInfo.tradingFee
          : 0
      }<br /><br /><b>Total: $${this.gasTradingTotal}</b>`;
    },
    isLogin() {
      return (
    ...
    gasTradingTotal() {
      ...
      return gasFee.plus(this.orbiterTradingFee).toFixed(2);
    },
    gasSavingMax() {
      let savingValue =
        this.$store.state.transferData.ethPrice * this.originGasCost -
        this.gasTradingTotal.toString();
      if (savingValue < 0) {
        savingValue = 0;
      }
      let savingTokenName = "$";
    ...
    "$store.state.transferData.fromChainID": function (newValue) {
      ...
      if (
        ...
        this.makerInfoList.filter((makerInfo) => {
          ...
          if (makerInfo.c1ID === fromChainID && makerInfo.c2ID === toChainID) {
            let isMatch = false;
          ...
          if (makerInfo.c2ID === fromChainID && makerInfo.c1ID === toChainID) {
            let isMatch = false;
            this.tokenInfoArray.filter((tokenInfo) => {
              if (tokenInfo.token === makerInfo.tName) {
                isMatch = true;
        ...
        console.log(this.$store.state.transferData.selectTokenInfo);
      ...
      if (selectMakerInfo && that.isLogin) {
        transferCalculate
          .getTransferBalance(
            ...
            selectMakerInfo.t1Address,
            selectMakerInfo.tName,
            that.$store.state.web3.coinbase
          )
          .then((response) => {
            that.c1Balance = (
              response /
