...
const rpcObject = env.localProvider;
for (const insideId in rpcObject) {
    const pubId = env.localChainID_netChainID[insideId];
    if (extraRpcs[pubId] && extraRpcs[pubId].rpcs) {
        extraRpcs[pubId].rpcs.push({
            url: rpcObject[insideId]
        });
    }
}
