// RENDER 80C
const fromRule = require('./eth-e4e-prod.json')
const toRule = require('./eth-80c-prod.json')
...
for (const chain in toRule) {
  for (const symbol in toRule[chain]) {
    console.log(chain, '====', symbol)
    if (fromRule[chain][symbol]) {
      const fromItem = fromRule[chain][symbol];
      toRule[chain][symbol]['gasFee'] = fromItem.gasFee;
      toRule[chain][symbol]['tradingFee'] = fromItem.tradingFee;
    }
  }
  console.log(JSON.stringify(toRule));
  ...
  //     for (const k in rule) {
  //       if (
  //         String(rule[k]).toLocaleLowerCase() === l1Maker.toLocaleLowerCase()
  //       ) {
  //         rule[k] = l1AfterMaker
  //       }
  //       if (
  //         String(rule[k]).toLocaleLowerCase() === l2Maker.toLocaleLowerCase()
  //       ) {
  //         rule[k] = l2AfterMaker
  //       }
  //     }
