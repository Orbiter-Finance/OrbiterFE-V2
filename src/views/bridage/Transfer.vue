  ...
  <div class="transfer-box">
      ...
      >
        <span
          :style="`margin-right:10px;color:${
            isNewVersion ? (!isLightMode ? '#22DED7' : '#4890FE') : '#888888'
          }`"
    ...
    <div class="info-box">
      ...
      <div v-if="isShowUnreachMinInfo" class="info-item">
        <svg-icon class="info-icon" iconName="info"></svg-icon>
        <span class="red"> Less than the minimum transfer amount. </span>
      </div>
      <div v-if="isShowMax" class="info-item">
        <svg-icon class="info-icon" iconName="info"></svg-icon>
        <span class="red">
          Makers provide {{ maxPrice }} {{ selectFromToken }} for liquidity.
        </span>
      </div>
...
export default {
  ...
  computed: {
    ...
    queryParams() {
      ...
      let source =
        makerConfigs.find((item) => item.fromChain.name === query.source)
          ?.fromChain?.id || 0
      let dest =
        makerConfigs.find((item) => item.toChain.name === query.dest)?.toChain
          ?.id || 0
      const getMapChainIds = (chainNames, isDest) => {
        ...
        for (const chainName of chainNames.split(',')) {
          const chainId = isDest
            ? makerConfigs.find((item) => item.toChain.name === chainName)
                ?.toChain?.id || 0
            : makerConfigs.find((item) => item.fromChain.name === chainName)
                ?.fromChain?.id || 0
          if (chainId) {
  ...
  methods: {
    ...
    async updateTransferInfo({
      ...
      let makerConfigList = makerConfigs.filter(
        (item) =>
          item.fromChain.id === fromChainID && item.toChain.id === toChainID
      )
    ...
    async updateSendBtnInfo() {
      ...
      let userMax =
        useBalance.decimalPlaces(availableDigit, BigNumber.ROUND_DOWN) > 0
    ...
    async updateUserMaxPrice() {
      ...
      let userMax =
        userBalance.decimalPlaces(avalibleDigit, BigNumber.ROUND_DOWN) > 0
      ...
      if (!walletIsLogin.value) {
        ...
        if (fromChainID === 4 || fromChainID === 44 || toChainID === 4 || toChainID === 44) {
          ...
          if (
            ...
            util.chainNetWorkId(fromChainID)
          ) {
    ...
    async getBalance(chainId, tokenAddress, tokenName, precision) {
      ...
      try {
        ...
        if (fromCurrency !== tokenName) {
          const exchangeRes = (
            await exchangeToCoin(response, tokenName, fromCurrency, this.rates)
          ).toString()
