    ...
    <div class="transfer-box">
        ...
        >
            <svg-icon
                ...
                iconName="tips"
            ></svg-icon>
            Centralized transfer is provided currently and trustless transfer
        ...
        <CommBtn
            ...
            :style="`border-radius: 40px;${
                !isNewVersion || isCrossAddress ? '' : 'margin-top: 10px'
            }`"
        >
        ...
        <div class="info-box">
            ...
            <div v-if="isShowMax" class="info-item">
                <svg-icon class="info-icon" iconName="info"></svg-icon>
                <span class="red">
                    Makers provide {{ maxPrice }} {{ selectFromToken }} for
                    liquidity.
                </span>
            </div>
        ...
        <CommTipDialog ref="TipPopupRef">
            <div slot="PoperContent" class="dialog">
                <div class="dialog-box">
                    ...
                    <div class="title">Unlock more Orbiter identities?</div>
                    <div class="content">
                        Explore more transactions on Orbiter Finance Mainnet
                        Have fun!
                    </div>
                    <div class="bottom">
                        <span class="btn" @click="openUrl"> Let's Go </span>
                    </div>
                </div>
            </div>
        </CommTipDialog>
...
export default {
    ...
    computed: {
        ...
        isCurrentAddress() {
            if (
                !this.isNewVersion ||
                this.selectFromToken === this.selectToToken
            ) {
                return false
            }
            if (
        ...
        queryParams() {
            ...
            let source =
                makerConfigs.find(
                    (item) => item.fromChain.name === query.source
                )?.fromChain?.id || 0
            let dest =
                makerConfigs.find((item) => item.toChain.name === query.dest)
                    ?.toChain?.id || 0
            const getMapChainIds = (chainNames, isDest) => {
                ...
                for (const chainName of chainNames.split(',')) {
                    const chainId = isDest
                        ? makerConfigs.find(
                              (item) => item.toChain.name === chainName
                          )?.toChain?.id || 0
                        : makerConfigs.find(
                              (item) => item.fromChain.name === chainName
                          )?.fromChain?.id || 0
                    if (chainId) {
            ...
            for (const tk of tokens) {
                const makerConfig = makerConfigs.find((item) =>
                    util.equalsIgnoreCase(item.fromChain.symbol, tk)
                )
                if (makerConfig) {
        ...
        isShowMax() {
            return (
                new BigNumber(this.transferValue).comparedTo(
                    new BigNumber(
                        transferDataState.selectMakerConfig?.fromChain?.maxPrice
                    )
                ) > 0
            )
        },
        isShowUnreachMinInfo() {
            if (walletIsLogin.value && this.transferValue) {
                const makerMin = new BigNumber(this.userMinPrice)
                const transferValue = new BigNumber(this.transferValue)
    ...
    methods: {
        ...
        refreshGasSavingMax() {
            ...
            const savingTokenName = '$'
            this.gasSavingMax =
                savingTokenName + savingValue.toFixed(2).toString()
        },
        refreshGasSavingMin() {
        ...
        refreshGasTradingTotal() {
            ...
            const gasFee = new BigNumber(selectMakerConfig.tradingFee)
            this.gasTradingTotal = gasFee
                .plus(this.orbiterTradingFee)
                .toFixed(6)
        },
        refreshGas() {
        ...
        async updateTransferInfo({
            ...
            const fromChainIdList = Array.from(
                new Set(makerConfigs.map((item) => item.fromChain.id))
            ).sort(function (a, b) {
                return a - b
            })
            fromChainID =
            ...
            toChainID =
                toChainID ||
                (dest && toChainIdList.find((item) => item === +dest)
                    ? +dest
                    : toChainIdList[0])
            if (toChainIdList.indexOf(toChainID) === -1) {
                toChainID =
                    toChainIdList.indexOf(dest) > -1 ? dest : toChainIdList[0]
            }
            ...
            let makerConfigList = makerConfigs.filter(
                (item) =>
            ...
            if (fromTokens.length) {
                makerConfigList = makerConfigList.filter((item) =>
                    fromTokens.find((it) =>
                        util.equalsIgnoreCase(it, item.fromChain.symbol)
                    )
                )
            }
            ...
            makerConfigList.forEach((item) => {
                if (
                    !fromTokenList.find(
                        (it) => it.token === item.fromChain.symbol
                    )
                ) {
                    fromTokenList.push({
                ...
                if (
                    fromCurrency === item.fromChain.symbol &&
                    !toTokenList.find((it) => it.token === item.toChain.symbol)
                ) {
                    toTokenList.push({
            ...
            if (
                ...
                !fromTokenList.find((item) => item.token === fromCurrency)
            ) {
            ...
            if (
                toTokenList.length &&
                !toTokenList.find((item) => item.token === toCurrency)
            ) {
            ...
            this.isShowExchangeIcon = !!makerConfigs.find(
                (item) =>
            ...
            const makerConfig = makerConfigs.find(
                (item) =>
                    item.fromChain.id === fromChainID &&
                    item.toChain.id === toChainID &&
            ...
            if (
                ...
                isCrossAddress &&
                makerConfigInfo.crossAddress?.recipient
            ) {
                makerConfigInfo.recipient =
                    makerConfigInfo.crossAddress?.recipient
                makerConfigInfo.sender = makerConfigInfo.crossAddress?.sender
                makerConfigInfo.tradingFee =
                    makerConfigInfo.crossAddress?.tradingFee
            ...
            if (fromChainID !== oldFromChainID) {
                ...
                transferCalculate
                    .transferSpentGas(
                        fromChainID,
                        this.$env.gasPriceMap,
                        this.$env.gasLimitMap
                    )
                    .then((response) => {
            ...
            if (
                fromChainID !== oldFromChainID ||
                fromCurrency !== oldFromCurrency
            ) {
        ...
        async updateSendBtnInfo() {
            const { selectMakerConfig, fromCurrency, toCurrency } =
                transferDataState
            if (!selectMakerConfig) return
            const { fromChain } = selectMakerConfig
            ...
            const opBalance = 10 ** -availableDigit
            const useBalance = new BigNumber(this.fromBalance)
            ...
            const userMax =
                ...
                0
                    ? useBalance.decimalPlaces(
                          availableDigit,
                          BigNumber.ROUND_DOWN
                      )
                    : new BigNumber(0)
            const makerMax = new BigNumber(fromChain.maxPrice)
            const makerMin = new BigNumber(this.userMinPrice)
            const transferValue = new BigNumber(this.transferValue || 0)
            const info = {
                text: 'SEND',
                disabled: null,
            }
            if (walletIsLogin.value) {
                ...
                if (transferValue.comparedTo(0) < 0) {
                    ...
                    info.disabled = 'disabled'
                    util.log(
                        'transferValue > userMaxPrice',
                        transferValue.toString(),
                        this.userMaxPrice.toString()
                    )
                }
                if (transferValue.comparedTo(userMax) > 0) {
                    ...
                    info.disabled = 'disabled'
                    util.log(
                        'transferValue < makerMin',
                        transferValue.toString(),
                        makerMin.toString()
                    )
                } else if (
                    transferValue.comparedTo(0) > 0 &&
                    this.toValue <= 0
                ) {
                    info.text = 'INSUFFICIENT FUNDS'
                    info.disabled = 'disabled'
                    util.log(
                        'transferValue > 0 && toValue <= 0',
                        transferValue.toString(),
                        this.toValue.toString()
                    )
                } else if (
                    this.toValue > 0 &&
                    this.toValue.comparedTo(
                        new BigNumber(this.makerMaxBalance)
                    ) > 0
                ) {
                    info.text = 'INSUFFICIENT LIQUIDITY'
                    info.disabled = 'disabled'
                    util.log(
                        'toValue > 0 && toValue > makerMaxBalance',
                        this.toValue.toString(),
                        new BigNumber(this.makerMaxBalance).toString()
                    )
                }
                ...
                if (this.isShowUnreachMinInfo || this.isShowMax) {
                    ...
                    info.disabled = 'disabled'
                    util.log(
                        'isShowUnreachMinInfo || isShowMax',
                        this.isShowUnreachMinInfo,
                        this.isShowMax
                    )
                }

                if (
                    (fromCurrency !== toCurrency || this.isCrossAddress) &&
                    !util.isSupportXVMContract() &&
                    !this.isLoopring &&
                    !util.isStarkNet()
                ) {
                    ...
                    info.disabled = 'disabled'
                    util.log(
                        '(fromCurrency !== toCurrency || this.isCrossAddress) && !isSupportXVMContract && !this.isLoopring && !util.isStarkNet',
                        fromCurrency !== toCurrency,
                        this.isCrossAddress,
                        !util.isSupportXVMContract(),
                        !this.isLoopring,
                ...
                if (
                    ...
                    info.disabled = 'disabled'
                    util.log(
                        'isSupportXVM && isCrossAddress && (!crossAddressReceipt || isErrorAddress)',
                        this.crossAddressReceipt,
                        this.isErrorAddress
                    )
                }
        ...
        updateRoutes(oldFromChainID, oldToChainID) {
            ...
            if (
                fromChainID !== oldFromChainID &&
                query?.source !== selectMakerConfig.fromChain.name
            ) {
                changeQuery.source = selectMakerConfig.fromChain.name
            }
            if (
                toChainID !== oldToChainID &&
                query?.dest !== selectMakerConfig.toChain.name
            ) {
        ...
        updateToValue() {
            const { fromCurrency, toCurrency, selectMakerConfig } =
                transferDataState
            ...
            if (fromCurrency !== toCurrency) {
                ...
                const value = amount
                    ...
                    .multipliedBy(toRate)
                    .toFixed(6)
                this.toValue = new BigNumber(value).multipliedBy(
                    1 - slippage / 10000
                )
            } else {
                this.toValue = amount
        ...
        async specialProcessing(oldToChainID) {
            ...
            if (
                (toChainID !== oldToChainID && oldToChainID === 4) ||
                oldToChainID === 44 ||
                oldToChainID === 11 ||
                oldToChainID === 511
            ) {
            ...
            if (
                ...
                if (!starkNetIsConnect || !starkNetAddress) {
                    ...
                    if (
                        ...
                        this.updateTransferInfo({
                            fromChainID: makerConfig.fromChain.id,
                        })
                        return
                    }
                }
            ...
            if (
                toChainID !== oldToChainID &&
                (toChainID === 11 || toChainID === 511)
            ) {
                if (!this.isCrossAddress) this.isCrossAddress = true
                const self = this
                if (
                    self.crossAddressReceipt !==
        ...
        async updateUserMaxPrice() {
            ...
            const avalibleDigit = orbiterCore.getDigitByPrecision(
                fromChain.decimals
            )
            ...
            const userBalance = new BigNumber(this.fromBalance)
                .minus(new BigNumber(selectMakerConfig.tradingFee))
                .minus(new BigNumber(opBalance))
            ...
            const userMax =
                ...
                0
                    ? userBalance.decimalPlaces(
                          avalibleDigit,
                          BigNumber.ROUND_DOWN
                      )
                    : new BigNumber(0)
            let max =
                userMax.comparedTo(new BigNumber(fromChain.maxPrice)) > 0
        ...
        addBalance(chainId, symbol, value, address) {
            const walletAddress =
                address ||
                compatibleGlobalWalletConf.value.walletPayload.walletAddress
            const addressBalanceMap = (this.balanceMap[walletAddress] =
                this.balanceMap[walletAddress] || {})
            const chainBalanceMap = (addressBalanceMap[chainId] =
                addressBalanceMap[chainId] || {})
      ...
      */
        ...
        transfer_mid() {
            const { fromChainID, toChainID, fromCurrency, toCurrency } =
                transferDataState
            this.updateTransferInfo({
                toChainID: fromChainID,
        ...
        getFromChainInfo(e) {
            if (transferDataState.fromChainID !== e.localID)
                this.updateTransferInfo({ fromChainID: e.localID })
        },
        getToChainInfo(e) {
            if (transferDataState.toChainID !== e.localID)
                this.updateTransferInfo({ toChainID: e.localID })
        },
        // open selectChain
        ...
        async sendTransfer() {
            ...
            if (!walletIsLogin.value) {
                ...
                if (!selectMakerConfig) return
                const { fromChain } = selectMakerConfig
                const nonce = await getNonce.getNonce(
                    fromChain.id,
                    fromChain.tokenAddress,
                    fromChain.symbol,
                    compatibleGlobalWalletConf.value.walletPayload.walletAddress
                ...
                if (
                    ...
                    if (!starkChain || starkChain === 'unlogin') {
                        util.showMessage(
                            'please connect StarkNet Wallet',
                            'error'
                        )
                        return
                    }
                        ...
                        if (
                            ...
                            if (matchSwitchChainDispatcher) {
                                ...
                                matchSwitchChainDispatcher(
                                    compatibleGlobalWalletConf.value
                                        .walletPayload.provider,
                ...
                const { isCrossAddress, crossAddressReceipt } =
                    transferDataState
                const walletAddress = (
                ...
                this.$store.commit('updateConfirmRouteDescInfo', [
                    {
                        ...
                        fromTip: '',
                        toTip: toAddressAll,
                        icon: util.isExecuteXVMContract()
                            ? 'contract'
                            : 'wallet',
                    },
                    {
        ...
        async updateOriginGasCost() {
            ...
            try {
                this.originGasCost =
                    await transferCalculate.transferOrginGasUsd(
                        fromChainID,
                        toChainID,
                        fromCurrency !== 'ETH'
                    )
        ...
        async updateExchangeToUsdPrice() {
            ...
            if (!selectMakerConfig) return
            const price = (
                await exchangeToUsd(1, selectMakerConfig.fromChain.symbol)
            ).toNumber()
            if (price > 0) {
                this.exchangeToUsdPrice = price
            }
        },
        async getBalance(chainId, tokenAddress, tokenName, precision) {
            ...
            try {
                ...
                if (fromCurrency !== tokenName) {
                    ...
                    return new BigNumber(exchangeRes)
        ...
        async getMakerMaxBalance() {
            ...
            const addressBalanceMap = (this.balanceMap[makerAddress] =
                this.balanceMap[makerAddress] || {})
            const chainBalanceMap = (addressBalanceMap[toChain.id] =
                addressBalanceMap[toChain.id] || {})
            ...
            if (_balance > 0) {
                ...
                this.makerMaxBalance = new BigNumber(_balance)
                    .multipliedBy(0.95)
                    .toString()
                this.addBalance(
                    toChain.id,
                    toChain.symbol,
                    this.makerMaxBalance,
                    makerAddress
                )
            }
        },
        gasCost() {
            ...
            if (
                ...
                const selectTokenRate = asyncGetExchangeToUsdRate(
            ...
            return (
                Math.ceil(
                    transferDataState.gasFee * transferDataState.ethPrice * 10
                ) / 10
            )
        },
        async refreshUserBalance() {
            ...
            if (address && address !== '0x') {
                const addressBalanceMap = (this.balanceMap[address] =
                    this.balanceMap[address] || {})
                const fromChainBalanceMap = (addressBalanceMap[fromChain.id] =
                    addressBalanceMap[fromChain.id] || {})
                if (
                    typeof fromChainBalanceMap[fromChain.symbol] === 'undefined'
                ) {
                    await transferCalculate
                        .getTransferBalance(
                            fromChain.id,
                            fromChain.tokenAddress,
                            fromChain.symbol,
            ...
            if (address && address !== '0x') {
                const toAddressBalanceMap = (this.balanceMap[address] =
                    this.balanceMap[address] || {})
                const toChainBalanceMap = (toAddressBalanceMap[toChain.id] =
                    toAddressBalanceMap[toChain.id] || {})
