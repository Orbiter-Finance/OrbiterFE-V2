      ...
      <div class="history-content">
        ...
        <div :hidden="currentNav !== 'Search'" style="margin: 20px">
          <el-form ref="formRef" label-width="140px">
            ...
            <el-form-item label="* From Chain">
              <el-select
                ...
                style="width: 70%; height: 30px"
                placeholder="Please select From Chain"
              >
                <el-option
          ...
          <div class="search">
            ...
            >
              <span style="letter-spacing: 0.15rem"> Submit </span>
            </CommBtn>
          </div>
          <div>
            <div v-if="showError" class="search" style="font-weight: bolder">
              No relevant destination Transaction Hash is found. You can submit
              your source Transaction Hash on the
              <span class="url" @click="openUrl('http://discord.gg/hJJvXP7C73')"
            ...
            <div class="search" style="margin-top: 20px">
              <span class="text" style="font-size: 16px"
                >To confirm that your Transaction Hash was generated from
                Orbiter, you can:</span
              ><br />
              <span class="text"
                >1. Make sure that this Transaction Hash was initiated at
                <span class="bold">Orbiter official website</span>.</span
              ><br />
              <span class="text"
                >2. The last four digits of your Tx value contain an ID code
                starts with <span class="bold">90XX</span>.</span
              ><br />
              <span class="text"
                >3. Your transaction <span class="bold">amount</span> is
                supported by Orbiter.</span
              >
            </div>
          </div>
        </div>
      ...
      if (status === 99) {
        ...
        for (const tx of txList) {
          ...
          if (tx.side === 0) {
            ...
            data.fromAmountValue = new BigNumber(tx.value)
          ...
          if (tx.side === 1) {
            ...
            data.toAmountValue = new BigNumber(tx.value)
              .dividedBy(10 ** decimal)
              .toFixed(8)
          }
        }
        ...
        await openApiAx.get(
          `/collectUserTransactions?fromHash=${txHash}&fromChain=${selectChainId}`
        )
        this.showError = true
      }
    },
