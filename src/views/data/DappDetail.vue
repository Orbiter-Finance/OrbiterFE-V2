                ...
                <div class="checker">
                    ...
                    >
                        <div
                            class="checkbox"
                            :class="{
                                active: checkInteractionsData.includes(item),
                            }"
                        ></div>
                        <div
                            class="line"
...
export default {
        ...
        chartData() {
            ...
            if (isMax(currentTime)) {
                return {
                    ...
                    chart_data_bridge: chart_data_bridge.sort(
                        (a, b) => a.timestamp - b.timestamp
                    ),
                }
            }
    ...
    watch: {
        chartData() {
            ...
            this._interactionsChart &&
                this._interactionsChart.setOption(
    ...
    methods: {
        ...
        async onRollupChange(value) {
            ...
            this.detailData = {}
            this.$loader.show()
            this.detailData = await getDappDetail(
                value,
                this.dappData.dapp_name
            )
                    ...
                    axisLabel: {
                        interval: (index) => {
                            return (
                                index %
                                    Math.ceil(
                                        times.length / (this.isMobile ? 3 : 6)
                                    ) ===
                                0
                            )
                        },
        ...
        _getInteractionsChartOptions() {
            const { times, daily_tx, bridge_tx, other_tx } =
                this._getInteractionsData()
        ...
        _onFormatter(params) {
            ...
            const all_users = this._getDataByTime(
                axisValue,
                'chart_data'
            ).all_users
        ...
        _onInteractionsFormatter(params) {
            const axisValue = params[0].axisValue
            const daily_tx = this._getDataByTime(
                axisValue,
                'chart_data_bridge'
            ).daily_tx
            const title = dateFormat(parseInt(axisValue), 'yyyy-MM-dd')
