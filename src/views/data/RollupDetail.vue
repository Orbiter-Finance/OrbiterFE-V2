          ...
          <span class="fee"
            >${{
              detailData.info.rollup_avg_fee == 0
                ? 0
                : Number(detailData.info.rollup_avg_fee).toFixed(2)
            }}
          </span>
          <span class="text">
            <!-- {{ detailData.info.rollup_category }} -->
      ...
      <div class="rollup_charts" v-if="detailData.info">
            ...
            <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
              <div class="content" style="margin-top: 0">
                <div class="title">
                  ...
                  >
                    <div class="supported-l2-desc">
                      Starknet Received on L2: Total gas received of every
                      successful transaction on Starknet Starknet Spent on L1:
                      Total gas used by Starknet's L1 contracts to settle L2
                      proof on Ethereum
                    </div>
                    <span class="title-help" slot="reference"> </span>
                  </el-popover>
        ...
        <div class="content" style="margin-top: 0">
          <div class="title">
            ...
            >
              <div class="supported-l2-desc">
                Transaction data using the official bridge and shows the amount
                of gas consumed by deposits in different currencies.
              </div>
              <span class="title-help" slot="reference"> </span>
            </el-popover>
          </div>
        ...
        <div class="content" style="margin-top: 0">
          <div class="title">
            ...
            >
              <div class="supported-l2-desc">
                Transaction data using the official bridge and shows the amount
                of gas consumed by withdraws in different currencies.
              </div>
              <span class="title-help" slot="reference"> </span>
            </el-popover>
          </div>
...
export default {
    ...
    chartData () {
      if (this.detailData.info) {
        ...
        this._chartInteractions &&
          this._chartInteractions.setOption(this._getInteractionsChartOptions())
        this._chartTransaction &&
          this._chartTransaction.setOption(this._getTransactionChartOptions())
        this._chartFees &&
          this._chartFees.setOption(this._getFeesChartOptions())
        this._chartDeposit &&
          this._chartDeposit.setOption(this._getDepositChartOptions())
        this._chartWithdraw &&
          this._chartWithdraw.setOption(this._getWithdrawChartOptions())
      }
    },
  },
  computed: {
  ...
  methods: {
    ...
    onCheckerClick (type, item) {
      if (type === 1) {
        ...
        if (this.checkInteractionsData.includes(item)) {
          this.checkInteractionsData = this.checkInteractionsData.filter(
            data => data !== item
          )
        } else {
          ...
          chartData[key].map(item => {
            for (const v in item) {
              if (!data[key][v]) {
                data[key][v] = []
                data[key][v].push(
                  v == 'timestamp' ? padTimestamp(item[v]) : item[v]
                )
              } else {
                data[key][v].push(
                  v == 'timestamp' ? padTimestamp(item[v]) : item[v]
            ...
            axisLabel: {
              interval: index => {
                return (
                  index %
                    Math.ceil(
                      chart_data_user.timestamp.length / (this.isMobile ? 3 : 6)
                    ) ===
                  0
                )
              },
            ...
            axisLabel: {
              ...
              interval: index => {
                return (
                  index %
                    Math.ceil(
                      chart_data_transaction.timestamp.length /
                        (this.isMobile ? 3 : 4)
                    ) ===
                  0
                )
            ...
            axisLabel: {
              ...
              interval: index => {
                return (
                  index %
                    Math.ceil(
                      chart_data_fee.timestamp.length / (this.isMobile ? 3 : 4)
                    ) ===
                  0
                )
              },
              ...
              axisLabel: {
                interval: index => {
                  return (
                    index %
                      Math.ceil(
                        chart_data_deposit.timestamp.length /
                          (this.isMobile ? 3 : 6)
                      ) ===
                    0
                  )
              ...
              axisLabel: {
                interval: index => {
                  return (
                    index %
                      Math.ceil(
                        chart_data_withdraw.timestamp.length /
                          (this.isMobile ? 3 : 6)
                      ) ===
                    0
                  )
    ...
    _onFormatter (params) {
      const axisValue = params[0].axisValue
      const all_users = ['All Users', 'Active Users', 'New Users'].includes(
        params[0].seriesName
      )
                       ...
                       return `
                        ...
                        ${
                          item.seriesName == 'Transactions'
                            ...
                            : `<div class="value">${numeral(item.value).format(
                                '0,0'
                              )}
                        ${
