    ...
    <div class="chart-wrapper">
        ...
        <div class="rollups">
            <div class="head">
                ...
                <time-diff
                    v-if="
                        !isMobile && baseChartData && baseChartData.update_time
                    "
            ...
            <div class="contents">
                <div class="content1">
                    ...
                    >
                        ...
                        <div
                            class="icon"
                            v-if="isShowDetail(item.name)"
                            @click="onRowClick(item.name)"
                        >
                            <svg-icon-themed
                                icon="searchChartIcon"
                                size="lg"
                            ></svg-icon-themed>
                        </div>
                        <div class="num">
...
export default {
    ...
    methods: {
            ...
            if ([3, 6].includes(currentChartTime)) {
                return {
                    ...
                    data: this.filteredChartData
                        .map((item) => item.all)
                        .reverse(),
                    data_l1: this.filteredChartData
                        .map((item) => item.all_l1)
                        .reverse(),
                }
            }
                    ...
                    currentChartTime === 12
                        ...
                        : padTimestamp(
                              this.filteredChartData[
                                  this.filteredChartData.length - 1
                              ].timestamp
                          )
                const endTime = padTimestamp(
                    this.filteredChartData[0].timestamp
                )
        ...
        _onFormatter([params, params2]) {
            ...
            const domMod = this.checkData.includes(
                'Ethereum Mainnet Transactions'
            )
                ? `<div class="chart-popover-total-transactions">
                    Ethereum Transactions: <span style='color: rgb(17, 112, 255)'>${numeral(
                        params2.data
                    ).format('0,0')}</span>
                  </div>`
                : ''
            return `<div class="chart-popover-content">
                <div class="chart-popover-title">${title}</div>
                    ${domMod}
                    <div class="chart-popover-total-transactions">
                    Layer2 Total Transactions: <span>${numeral(
                        params.data
                    ).format('0,0')}</span>
                    </div>
