...
export default {
    ...
    async transferSpentGas(fromChainID, gasPriceMap, gasLimitMap) {
        ...
        if (fromChainID === 4 || fromChainID === 44) {
            let realTransferAmount = this.realTransferAmount().toString();
            let fromTokenAddress = selectMakerConfig.fromChain.tokenAddress;
            let starkFee = await getStarkTransferFee(
                web3State.coinbase,
                fromTokenAddress,
                selectMakerConfig.recipient,
    ...
    async transferOrginGasUsd(fromChainID, toChainID, isErc20 = true) {
        ...
        if (fromChainID === 12 || fromChainID === 512) {
            try {
                ...
                ethGas += Number(zkspaceWithDrawFee * 10 ** selectMakerConfig.fromChain.decimals);
            } catch (error) {
    ...
    async getTransferBalance(
        ...
        if (localChainID === 3 || localChainID === 33) {
            ...
            try {
                ...
                const zksTokenInfos = localChainID === 12
    ...
    realTransferAmount() {
        const { selectMakerConfig, transferValue, fromChainID, toChainID } = transferDataState;
        let userValue = new BigNumber(transferValue).plus(
