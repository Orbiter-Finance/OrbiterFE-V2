...
export default {
    ...
    async getTransferGasLimit(fromChainID, makerAddress, fromTokenAddress) {
        ...
        if (fromChainID === 3 || fromChainID === 33) {
            ...
            try {
                ...
                if (
                    !addressState.committed ||
                    addressState.committed?.nonce == 0
                ) {
                    const changePubKeyFee =
                        await syncHttpProvider.getTransactionFee(
            ...
            const estimateGas = await util.requestWeb3(
                fromChainID,
                'estimateGas',
                {
    ...
    async transferOrginGasUsd(fromChainID, toChainID, isErc20 = true) {
        ...
        if (fromChainID === 2 || fromChainID === 22) {
            try {
                ...
                const ARWithDrawL1Gas =
                    L1GasPrice *
                    (isErc20 ? AR_ERC20_WITHDRAW_ONL1 : AR_ETH_WITHDRAW_ONL1)
        ...
        if (fromChainID === 7 || fromChainID === 77) {
            try {
                ...
                const OPWithDrawOPGas =
                    fromGasPrice *
                    (isErc20
                        ? OP_ERC20_WITHDRAW_ONOP_L2
                        : OP_ETH_WITHDRAW_ONOP_L2)
                ...
                const OPWithDrawL1Gas =
                    L1GasPrice *
                    (isErc20 ? OP_ERC20_WITHDRAW_ONL1 : OP_ETH_WITHDRAW_ONL1)

                const OPWithDrawOP_L1 = await this.getOPFee(fromChainID)

                ethGas =
                    OPWithDrawOPGas + OPWithDrawL1Gas + Number(OPWithDrawOP_L1)
        ...
        if (fromChainID === 12 || fromChainID === 512) {
            try {
                const zkspaceWithDrawFee =
                    await zkspace.getZKSpaceWithDrawGasFee(
                        fromChainID,
                        web3State.coinbase
                            ? web3State.coinbase
                            : selectMakerConfig.recipient
        ...
        if (fromChainID === 16 || fromChainID === 516) {
            try {
                ...
                // AR WithDraw
                const ARWithDrawARGas =
                    fromGasPrice * (isErc20 ? 300000 : 65000)
        ...
        if (toChainID === 3 || toChainID === 33) {
            try {
                ...
                const zkDepositGas =
                    toGasPrice *
                    (isErc20
                        ? ZK_ERC20_DEPOSIT_APPROVEL_ONL1 +
                          ZK_ERC20_DEPOSIT_DEPOSIT_ONL1
        ...
        if (toChainID === 11 || toChainID === 511) {
            try {
                ...
                const dydxDepositGas =
                    toGasPrice * DYDX_ETH_DEPOSIT_DEPOSIT_ONL1
    ...
    async getTransferBalance(
        ...
        if (localChainID === 3 || localChainID === 33) {
            ...
            const balance = await imxHelper.getBalanceBySymbol(
                userAddress,
                tokenName
            )
    ...
    async getBalanceByRPC(chainId, userAddress, tokenAddress) {
        if (util.isEthTokenAddress(chainId, tokenAddress)) {
            // When is ETH
            const balance = await util.requestWeb3(
                chainId,
                'getBalance',
                userAddress
            )
    ...
    getTransferTValue() {
        ...
        return orbiterCore.getTAmountFromRAmount(
            fromChainID,
            rAmountValue,
            p_text
        )
