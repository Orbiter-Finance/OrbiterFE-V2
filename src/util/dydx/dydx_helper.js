...
export class DydxHelper {
    ...
    async getDydxClient(
        ...
        if (clientOld && !alwaysNew) {
            if (alwaysDeriveStarkKey && ethereumAddress) {
                ...
                clientOld.starkPrivateKey =
                    await clientOld.onboarding.deriveStarkKey(
                        ethereumAddress,
                        this.signingMethod
                    )
            }
        ...
        if (ethereumAddress && this.web3) {
            ...
            if (userExists.exists) {
                if (alwaysDeriveStarkKey) {
                    client.starkPrivateKey =
                        await client.onboarding.deriveStarkKey(
                            ethereumAddress,
                            this.signingMethod
                        )
                }
    ...
    static toTransaction(transfer, ethereumAddress) {
        ...
        const isTransferOut = util.equalsIgnoreCase(
            'TRANSFER_OUT',
            transfer.type
        )
