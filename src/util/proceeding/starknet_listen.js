                ...
                for (const tx of transactions.filter(
                    ...
                    const from = starknet.number.toHex(
                        starknet.number.toBN(
                            starknet.number.hexToDecimalString(
                                tx.contract_address
                            )
                        )
                    )
                    const to = starknet.number.toHex(
                        starknet.number.toBN(calldata[6])
                    )
                    ...
                    for (const item of this.listens) {
                        ...
                        if (
                            this.transferReceivedHashs[tx.transaction_hash] !==
                            true
                        ) {
                            this.transferReceivedHashs[
                                tx.transaction_hash
                        ...
                        if (
                            ...
                            console.warn(
                                `Transaction [${transaction.hash}] was confirmed.`
                            )
                            if (clearTiker) {
