...
import { CircleAlert, CircleCheck, CircleX, Info, SquareArrowOutUpRightIcon, XIcon } from 'lucide-react'
...
export function OrbiterToastCard({
    ...
    switch (type) {
        case TOAST_STATUS.ERROR:
            ...
            icon = <CircleX className='w-5 h-5' stroke='var(--o-color-brand-500)' />
            break
        case TOAST_STATUS.SUCCESS:
            color = "text-[var(--o-color-green-500)]"
            bgColor = "bg-[var(--o-color-green-900)]"
            icon = <CircleCheck className='w-5 h-5' stroke='var(--o-color-green-500)' />
            break
        case TOAST_STATUS.WARN:
            color = "text-[var(--o-color-yellow-500)]"
            bgColor = "bg-[var(--o-color-yellow-900)]"
            icon = <CircleAlert className='w-5 h-5' stroke='var(--o-color-yellow-500)' />
            break
    ...
    return (
        <div className={
            cn('p-3 flex rounded-xl sm:rounded-2xl justify-between items-start', pageIsMobile ? `${bgColor} ${color} w-full` : "bg-[var(--o-color-gray-700)] max-w-[30rem]")
        }>
            <div className='flex justify-start items-start flex-1 w-full'>
                ...
                <div className='flex-1 text-left'>
                    ...
                    <OrbiterShow
                        when={!!viceTitle && !pageIsMobile}
                    >
                        <div className='w-full mt-2 text-sm break-all w-full'>{viceTitle}</div>
                    </OrbiterShow>
                    <OrbiterShow
                        when={!!linkLabel && !pageIsMobile}
                    >
                        <div onClick={(event) => {
                            ...
                            {linkLabel}
                            <SquareArrowOutUpRightIcon stroke='var(--o-color-blue-500)' className='w-3 h-3' />
                        </div>
                    </OrbiterShow>
                </div>
            </div>
